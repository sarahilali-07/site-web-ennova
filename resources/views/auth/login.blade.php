...
<section class="min-h-[calc(100vh-4rem)] flex items-center justify-center py-20">
    <div class="max-w-lg w-full bg-dark border border-dark-border rounded-[2rem] p-10 shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
        <div class="mb-8 text-center">
            <h1 class="text-4xl font-bold text-white">{{ __('messages.auth.login.title') }}</h1>
            <p class="text-gray-400 mt-2">{{ __('messages.auth.login.title') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5" autocomplete="off">
            ...
            <div class="flex items-center justify-between text-sm text-gray-400">
                <label class="inline-flex items-center gap-2">
                    ...
                    {{ __('messages.auth.login.remember') }}
                </label>
                    <a href="{{ route('home') }}" class="text-orange hover:text-orange-light">{{ __('messages.general.back') }}</a>
            </div>
