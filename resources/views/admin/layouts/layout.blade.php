...
<head>
    ...
    <title>@yield('title', 'Dashboard') | Ennova Admin</title>
    ...
    <style>
        * { font-family: 'DM Sans', sans-serif; }
        ...
        html:not(.dark) .sidebar-link { color: #6B7280; }
...
<div class="flex min-h-screen">
    {{-- Sidebar --}}
    <aside class="w-60 border-r border-dark-border dark:border-dark-border flex-shrink-0 flex flex-col transition-colors dark:bg-dark-card bg-white">
        <div class="p-5 border-b border-dark-border dark:border-dark-border">
    ...
    >
        ...
        <nav class="flex-1 p-4 space-y-1">
            <p class="text-gray-600 dark:text-gray-600 text-xs uppercase tracking-wider px-3 py-2 font-semibold">Principal</p>
            ...
            <p class="text-gray-600 dark:text-gray-600 text-xs uppercase tracking-wider px-3 py-2 mt-4 font-semibold">Contenu</p>
        ...
        <div class="p-4 border-t border-dark-border dark:border-dark-border">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-gray-500 dark:text-gray-500 text-xs">Admin</p>
                </div>
            </div>
            <div class="flex space-x-2">
                <a href="{{ route('home') }}" class="flex-1 text-center text-xs text-gray-600 dark:text-gray-500 hover:text-orange dark:hover:text-orange py-2 border border-light-border dark:border-dark-border rounded-lg transition-colors">Site</a>
                <form method="POST" action="{{ route('logout') }}" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full text-xs text-gray-600 dark:text-gray-500 hover:text-red-400 dark:hover:text-red-400 py-2 border border-light-border dark:border-dark-border rounded-lg transition-colors">Déco.</button>
                </form>
            </div>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="border-b border-dark-border dark:border-dark-border px-6 py-4 flex items-center justify-between dark:bg-dark-default bg-white transition-colors" style="background: rgba(250,250,250,0.5); --tw-bg-opacity: 0.5;">
            <h1 class="font-display font-bold text-gray-900 dark:text-white text-xl dark:text-xl">@yield('page-title', 'Dashboard')</h1>
            <div class="flex items-center space-x-4">
                @if(session('success'))
                    <div class="bg-green-900/50 dark:bg-green-900/50 border border-green-700/50 dark:border-green-700/50 text-green-400 dark:text-green-400 px-4 py-2 rounded-lg text-sm">{{ session('success') }}</div>
                @endif
                @if(session('error'))
                    <div class="bg-red-900/50 dark:bg-red-900/50 border border-red-700/50 dark:border-red-700/50 text-red-400 dark:text-red-400 px-4 py-2 rounded-lg text-sm">{{ session('error') }}</div>
                @endif
                <div class="text-gray-500 dark:text-gray-500 text-sm">{{ now()->format('d M Y') }}</div>
                
                {{-- Theme Toggle --}}
                <button data-theme-toggle class="text-gray-600 dark:text-gray-400 hover:text-orange dark:hover:text-orange transition-colors p-2.5 rounded-lg dark:bg-dark-card bg-light-card border border-light-border dark:border-dark-border hover:border-orange/30 ml-2">
